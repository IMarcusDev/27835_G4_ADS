# Matriz de Trazabilidad IREB

Matriz de trazabilidad de requisitos según el estándar IREB (International Requirements Engineering Board).

## Versiones y Variantes

| **Código del ECS** | **Nombre completo del ECS**   | **Autor(es) del artefacto**              | **Ruta en el repositorio Git**                                     | **Línea base a la que pertenece** | **Tipo de ECS** | **Fecha de creación** | **Última fecha de modificación** |
| ------------------ | ----------------------------- | ---------------------------------------- | ------------------------------------------------------------------ | --------------------------------- | --------------- | --------------------- | -------------------------------- |
| MTZ                | Matriz de Trazabilidad IREB   | Marcos Escobar, Mateo Sosa, Fernando Tipán | 27835_G4_ADS/Biblioteca de Trabajo/1. ELICITACION/1.3 Matriz IREB/ | LBR                               | Documentación   | 25/11/2025            | 25/11/2025                       |

## Descripción del Artefacto

La matriz IREB permite la trazabilidad bidireccional entre:
- Requisitos de usuario y requisitos del sistema
- Requisitos funcionales y casos de uso
- Requisitos y componentes de diseño
- Requisitos y casos de prueba

## Historial de Cambios

| **Versión** | **Fecha**   | **Descripción del Cambio**                          | **Autor**   |
| ----------- | ----------- | --------------------------------------------------- | ----------- |
| V1.0        | 18/11/2025  | Versión inicial de la matriz IREB                   | Equipo G4   |
| V2.0        | 20/11/2025  | Actualización de trazabilidad de requisitos         | Equipo G4   |
| V3.0        | 25/11/2025  | Versión final con trazabilidad completa             | Equipo G4   |

## Archivos

| **Archivo**                        | **Versión** | **Descripción**                              |
| ---------------------------------- | ----------- | -------------------------------------------- |
| G4_27835_Matriz_IREBv1.0.xlsx      | V1.0        | Versión inicial de la matriz de trazabilidad |
| G4_27835_Matriz_IREBv2.0.xlsx      | V2.0        | Actualización de trazabilidad                |
| G4_27835_Matriz_IREBv3.0.xlsx      | V3.0        | Versión actual de la matriz de trazabilidad  |

## Control de Configuración
- **Verificado por:** Mateo Sosa (Gestor de Configuración)
- **Fecha de verificación:** 24/01/2026
- **Estado:** Trazabilidad de versiones verificada
